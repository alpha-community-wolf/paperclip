  ...
  const svc = {
    ...
    maybeAutoCaptureMemos: async (opts: {
      ...
      try {
        ...
        const chorePrompt = [
          "You are a company knowledge extraction system. Read the issue comments below from a completed agent run.",
          "Extract 0-3 pieces of COMPANY KNOWLEDGE — facts about the business, its partners, clients, strategies, operational decisions, or team preferences that a new team member would need to know.",
          "",
          "Categories (pick one per memory):",
          '- fact: verifiable truth about the company, its partners, clients, or industry (e.g. "Site Eye provides cameras to the Kayamandi control room at no charge")',
          '- decision: a strategic or operational choice made by leadership or the team (e.g. "We chose React over Svelte for the dashboard")',
          '- lesson_learned: a business or operational insight others should know (e.g. "Developer opinion posts need lived experience to perform well")',
          '- procedure: a company process or workflow (e.g. "All PRs require Michael\'s review before merge")',
          "",
          "EXCLUDE (do not extract):",
          "- Code patterns, API quirks, library behavior, or debugging tricks (these belong in code comments, not company knowledge)",
          "- UI component behavior, CSS fixes, or frontend implementation details",
          "- Build tool configs, dependency management, or dev environment setup",
          "- Internal platform/infrastructure implementation details",
          "- Status updates, progress reports, and routine operational comments",
          "",
          "Litmus test: Would a new team member need to know this about the COMPANY (not the codebase)? If it's about how code works rather than how the business works, skip it.",
          "",
          "Rules:",
          "- Each memory must be a single concise sentence (under 200 chars).",
          "- If nothing qualifies as company knowledge, return an empty array.",
